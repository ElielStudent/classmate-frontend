...
const sidebarItems: SidebarItem[] = [
  { name: 'Find Sessions', href: '/student/find-sessions', icon: HomeIcon },
  { name: 'Upcoming Sessions', href: '/student/upcoming-sessions', icon: CalendarIcon },
  { name: 'Session History', href: '/student/session-history', icon: ClockIcon },
];
  ...
  const Header = () => (
    <header className="bg-green-300 dark:bg-gray-800 shadow-sm sticky top-0 z-30">
      <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8 flex justify-between items-center">
        ...
        <h1 className="text-lg font-semibold text-gray-800 dark:text-white">Student Dashboard</h1>
        <DarkModeToggle />
      </div>
    </header>
  );
  ...
  const Sidebar = () => (
    <>
      ...
      <div className={`lg:hidden fixed inset-x-0 top-[57px] bottom-0 z-20 transition-opacity duration-300 ${sidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`bg-green-600 text-white w-full py-6 px-4 transition-transform duration-300 ease-in-out transform ${sidebarOpen ? 'translate-y-0' : '-translate-y-full'}`}>
          <SidebarContent />
        </div>
      </div>

      {/* Sidebar for desktop */}
      <div className="hidden lg:block bg-green-600 text-white w-64 space-y-6 py-7 px-2 h-screen fixed left-0 top-0 overflow-y-auto">
        <SidebarContent />
      </div>
    </>
  );
  ...
  const SidebarContent = () => (
    <nav className="space-y-2">
      {sidebarItems.map((item) => (
        <Link key={item.name} href={item.href} className="block py-2.5 px-4 rounded transition duration-200 hover:bg-green-700">
